...
func HandleGet(cmds []string, index int, isReplay bool) []byte {
	...
	if data.kind != "string" || data.strVal == nil {
		return resp.ErrorResponse("value not a string")
	}
...
func HandleDel(cmds []string, index int, isReplay bool) []byte {
	...
	for i := 1; i < len(cmds); i++ {
		key := cmds[i]
		keyIdx := shardIndex(key, NumShards)
		shardLocks[keyIdx].Lock()
		removeIfExpired(key, keyIdx)
		if _, ok := shards[keyIdx][key]; ok {
			incKeyVersion(key)
			delete(shards[keyIdx], key)
			removedNbr++
		}
		shardLocks[keyIdx].Unlock()
	}
	if removedNbr > 0 {
		logToAOF(cmds, isReplay)
	}
...
func HandleExists(cmds []string, index int, isReplay bool) []byte {
	...
	for i := 1; i < len(cmds); i++ {
		...
		keyIdx := shardIndex(key, NumShards)
		shardLocks[keyIdx].Lock()
		if isExpired(key, keyIdx) {
			shardLocks[keyIdx].Unlock()
			continue
		}
		_, ok := shards[keyIdx][key]
		if ok {
			nbrExist++
		}
		shardLocks[keyIdx].Unlock()
...
func HandleHSet(cmds []string, index int, isReplay bool) []byte {
	if (len(cmds)-2)%2 != 0 {
		return resp.ErrorResponse(fmt.Sprintf("wrong number of arguments for '%s' command", cmds[0]))
	}
	...
	for i := 2; i < len(cmds); i += 2 {
		field := cmds[i]
		val := cmds[i+1]
		if old, exists := (*data.hashVal)[field]; !exists || old != val {
        	changed = true
    	}
